...
import searchbyimagefromatals from './lib/searchbyimagefromatlas.js';
import searchByUrlfromatlas from './lib/searchbyUrlfromatlas.js';
...
(async () => {
  try {
    ...
    server.post(
      '/api/searchWebcam',
      wrap(async (req: express.Request, res: express.Response, next) => {
    ...
    server.post(
      '/api/searchWebcamByURL',
      wrap(async (req: express.Request, res: express.Response, next) => {
        const query = { imageUrl: req.body.imageUrl, count: req.body.count };
        //searchByUrl(query, embedding, index, image_server).then(result => {
        searchByUrlfromatlas(query, embedding, image_server).then(result => {
          res.send(result);
        });
      })
    );
    ...
    server.post(
      ...
      wrap(async (req: express.Request, res: express.Response, next) => {
        ...
        searchbyimagefromatals(blobImage, count, embedding, image_server).then(result => {
          res.send(result);
        });
      })
    );
